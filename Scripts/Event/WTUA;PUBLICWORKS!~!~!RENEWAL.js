 ...
 * Record Type:- WTUA;PUBLICWORKS!*!*!RENEWAL.js
 *
...
//wfstatus=Approved/Denied
logDebug("Executing Script WorkflowTaskUpdateAfter4Renew");
// For CoD testing
// added an additional renewal task for CoD lines 209, 250
// -----------------------------------
// Transaction section.
// -------------------------------------------------------------------------------------------------
...
logDebug("1. Check to see if the license reviewing was approved by agency user.");
if ((wfTask=="Permit Issuance" || wfTask=="Renewal Review") && (wfStatus == "Issued" || wfStatus=="Approved")) {
    ...
    if (parentLicenseCAPID != null) {
        logDebug("3. Check to see if license is ready for renew.");
        if (isReadyRenew(parentLicenseCAPID)) {
            logDebug("3.1 Get projectScriptModel of renewal CAP.");
            renewalCapProject = getRenewalCapByParentCapIDForReview(parentLicenseCAPID);
            if (renewalCapProject != null) {
                logDebug("4. Set B1PERMIT.B1_ACCESS_BY_ACA to \"N\" for partial CAP to not allow that it is searched by ACA user.");
                aa.cap.updateAccessByACA(capID, "N");
                logDebug("5. Set parent license to \"Active\"");
                if (activeLicense(parentLicenseCAPID)) {
                    ...
                    logDebug("8. move renew document to parent cap");
                    ...
                    logDebug("9. Send approved license email to public user");
    	...
    	var parentLicenseCAPID = getParentCapID4Renewal(capID);
    	aa.print("parent capid :" + parentLicenseCAPID);
    	if (parentLicenseCAPID != null) {
...
function isApproveEtisalatPayment(capID, wfTask, stepNum, processID, taskStatus) {
    ...
    if (taskItem.getTaskDescription().equals(wfTask) && "Approve-Payment".equals(wfTask) && "Approve".equals(taskStatus)) {
        return true;
    }
    return false;
}
function isVoidEtisalatPayment(capID, wfTask, stepNum, processID, taskStatus) {
    ...
    if (taskItem.getTaskDescription().equals(wfTask) && "Approve-Payment".equals(wfTask) && "Void".equals(taskStatus)) {
        return true;
    }
    return false;
}
function isWorkflowApproveForReview(capID, wfTask, stepNum, processID, taskStatus) {
    ...
    if(result.getSuccess()) {
        ...
        if (("Permit Issuance" == wfTask || "Renewal Review" == wfTask) && ("Issued" == wfStatus || "Approved" == wfStatus)) {
            return true;
        } else {
            //aa.print("Issuing license (" + wfTask +") don't have been approved");
            logDebug("Issuing license (" + wfTask + ":" + wfStatus + ") don't have been approved");
        }
    } else {
...
function isWorkflowDenyForReview(capID, wfTask, stepNum, processID, taskStatus) {
    ...
    if(result.getSuccess()) {
        ...
        if (taskItemScriptModel.getTaskDescription().equals(wfTask)
            && ("Renewal Review".equals(wfTask) || "Permit Issuance".equals(wfTask)) //added an additional renewal task for CoD
            && "Denied".equals(wfStatus)) {
            return true;
        } else {
            aa.print("Issuing license (" + wfTask +") have been denied");
        }
    } else {
        aa.print("ERROR: Failed to get workflow task(" + capID + ") for review: " + result.getErrorMessage());
    }
...
function getRenewalCapByParentCapIDForReview(parentCapid) {
    ...
    //1. Get parent license for review
    var result = aa.cap.getProjectByMasterID(parentCapid, "Renewal", "");
    if(result.getSuccess()) {
        projectScriptModels = result.getOutput();
        if (projectScriptModels == null || projectScriptModels.length == 0) {
