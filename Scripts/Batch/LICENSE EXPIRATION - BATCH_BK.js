...
aa.env.setValue("fromDate", "12/01/2016");
//aa.env.setValue("toDate", "12/01/2016");*/
//aa.env.setValue("appGroup", "Licenses");
/*aa.env.setValue("appTypeType", "Mobile-Unit");
...
*/
//aa.env.setValue("skipAppStatus", "Closed,Confirmed Closed,Denied,Enforcement,Out of County MU,Pending,Surveillance,Suspended,Terminated,Voided,Withdrawn,Withheld");
/*------------------------------------------------------------------------------------------------------/
|
...
var fromDate = "01/01/2015";   //getParam("");//""; //							// Hardcoded dates.   Use for testing only
var toDate = "12/31/2017";  	//""; // 							// ""
...
var expStatus = "Active";     //getParam("expirationStatus");					//   test for this expiration status		 
var newExpStatus =  getParam("newExpirationStatus");				//   update to this expiration status  "About to Expire";
var newAppStatus =  getParam("newApplicationStatus");				//   update the CAP to this status    "About to Expire"; //
...
var skipAppStatusArray = "Closed,Confirmed Closed,Denied,Enforcement,Out of County MU,Pending,Surveillance,Suspended,Terminated,Voided,Withdrawn,Withheld"; //skipAppStatus.split(","); //getParam("skipAppStatus").split(",");	//   Skip records with one of these application statuses
var emailAddress = "sallami@detroitmi.gov"; // getParam("emailAddress");					// email to send report
var sendEmailToContactTypes =  getParam("sendEmailToContactTypes");// send out emails?    "Applicant";
var emailTemplate = "LICENSE ABOUT TO EXPIRE"; //getParam("emailTemplate");					// email Template. E.g., LICENSE ABOUT TO EXPIRE 90 DAYS
var createRenewalRecord = "Yes";   // create a temporary record    //getParam("Yes");	
...
var maxEntries = 9999;			// Maximum number of std choice entries.  Entries must be Left Zero Padded
...
var mailFrom = "noreply@accela.com";// lookup("ACA_EMAIL_TO_AND_FROM_SETTING", "RENEW_LICENSE_AUTO_ISSUANCE_MAILFROM");
var acaSite = lookup("ACA_CONFIGS", "ACA_SITE");
acaSite = acaSite.substr(0, acaSite.toUpperCase().indexOf("/ADMIN"));

//logDebug("Licenses: " + lookup("AGENCY_CONTACT_INFO", "01_Env"));
logDebug("Date Range -- fromDate: " + fromDate + ", toDate: " + toDate);

var startTime = startDate.getTime();			// Start timer
var systemUserObj = aa.person.getUser("ADMIN").getOutput();

if (appGroup == "Licenses") {
    appGroup = "Licenses";
}
if (appTypeType == "") {
    appTypeType = "*";
...
//    }
    ...
    for (thisExp in myExp) {
        ...
        //global variable needed for runASAForCapID function
        ...
        if (createRenewalRecord && createRenewalRecord.substring(0, 1).toUpperCase().equals("Y")) {
            ...
            logDebug("          " + "Could not create renewal record : " + createResult.getErrorMessage());
                ...
                if (renewalCap.isCompleteCap()) {
                    ...
                    //add fees 
//                    runASAForCapID(renewalCapId); //this simulates ASIUA or ASA run
                }
            }
...
//            }
            ...
            //if (conEmail && conEmail != "" && !exists(conEmail, conEmailArray) && vExOptIn == "CHECKED") {
                ...
                //var thisEmail = vConObj.people.getEmail();
...
//            if (vReport != false) {
//                [0] = vReport; // attaches the invoice to the email
//        }
            logDebug("mailFrom " + mailFrom + "; thisEmail " + thisEmail + "; emailTemplate " + emailTemplate + "; vEParams" + vEParams + "; capId4Email " + capId4Email);
            aa.document.sendEmailAndSaveAsDocument(mailFrom, thisEmail, "", emailTemplate, vEParams, capId4Email, "");
