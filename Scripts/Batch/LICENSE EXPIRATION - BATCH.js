...
eval(getMasterScriptText("INCLUDES_ACCELA_FUNCTIONS_ASB"));
...
if (!toDate.length) {// no "to" date, assume today + number of look ahead days + span
    toDate = dateAdd(null, parseInt( ) + parseInt(daySpan));
}
var mailFrom = lookup("ACA_EMAIL_TO_AND_FROM_SETTING", "RENEW_LICENSE_AUTO_ISSUANCE_MAILFROM");
var acaSite = lookup("ACA_CONFIGS", "ACA_SITE");
acaSite = acaSite.substr(0, acaSite.toUpperCase().indexOf("/ADMIN"));

logDebug("Environment: " + lookup("", ""));
    ...
    for (thisExp in myExp) {
        ...
        for (z in conTypeArray) {
            ...
            if (conEmail && conEmail != "" && !exists(conEmail, conEmailArray) && vExOptIn == "CHECKED") {
                ...
                if (conEmailList == "") {
                    conEmailList = conEmail;
                }
                else {
                    conEmailList = conEmailList + ", " + conEmail;
                }

                //logDebug("sendEmails " + sendEmails);
                logDebug("conEmailList " + conEmailList);
...
//                    }
            ...
            //aa.document.sendEmailAndSaveAsDocument(mailFrom, conEmailArray[y], "", emailTemplate, vEParams, capId4Email, fileNames);
            aa.document.sendEmailAndSaveAsDocument(mailFrom, conEmailList, "", emailTemplate, vEParams, capId4Email, fileNames);
        ...
        //    }
        //    //aa.document.sendEmailAndSaveAsDocument(mailFrom, conEmailArray[y], "", emailTemplate, vEParams, capId4Email, fileNames);
        //    aa.document.sendEmailAndSaveAsDocument(mailFrom, sendEmails, "", emailTemplate, vEParams, capId4Email, fileNames);
        //    logDebug("          " + altId + ": Sent Email template " + emailTemplate + " to " + conEmailArray[y]);
        //    emailCount++;
        //    sentEmail = true;
        //}
